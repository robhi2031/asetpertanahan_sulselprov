    ...
    protected function get_userinfo($username)
    {
        ...
        if($getRow==true){
            ...
            if($thumb==''){
                ...
                if (!file_exists(public_path(). '/dist/img/users/'.$thumb)){
                    $getRow->url_thumb = NULL;
                    $getRow->thumb = NULL;
                }else{
                    $getRow->url_thumb = url('dist/img/users/'.$thumb);
                }
            }
            //Get Roles
            $role = Role::select('roles.*')
                ->join('users_roles AS b', 'roles.id', '=', 'b.role_id', 'LEFT OUTER')
                ->where('b.user_id', $getRow->id)->first();
